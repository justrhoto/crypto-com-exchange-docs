...
URL: https://exchange-developer.crypto.com/exchange/v1/docs/api/websocket/ws-channel-user-advance-order-instrument-name

- **Method:** `POST`
- **Path:** `/ws/channel/user-advance-order-instrument-name`
- **Tags:** Websocket User API

OTO/OTOCO order updates. Subscribe with channel ["user.advance.order.{instrument_name}"].

## Request Body

### Parameters

- `channels` (array of string **required**) - e.g. ["user.advance.order.BTCUSD-PERP"]
...
openapi: 3.0.3
info:
  title: user.advance.order.{instrument_name}
  version: 1.0.0
servers:
...
paths:
  /ws/channel/user-advance-order-instrument-name:
    post:
      operationId: ws-channel-user-advance-order-instrument-name
      summary: user.advance.order.{instrument_name}
      description: OTO/OTOCO order updates. Subscribe with channel ["user.advance.order.{instrument_name}"].
      tags:
        - Websocket User API
      x-type: channel
      x-channel-url: wss://stream.crypto.com/exchange/v1/user
      x-channel-group: user
      x-name: user.advance.order.{instrument_name}
      requestBody:
        required: true
        content:
          application/json:
                ...
                params:
                  ...
                  properties:
                    channels:
                      ...
                      items:
                        type: string
                      description: e.g. ["user.advance.order.BTCUSD-PERP"]
                      example:
                        - user.advance.order.BTCUSD-PERP
                nonce:
                  type: integer
      ...
      responses:
        "200":
          ...
          content:
            application/json:
              schema:
                $ref: "#/components/schemas/WsUserAdvancedOrderPayload"
              example:
                id: 1
                method: subscribe
                code: 0

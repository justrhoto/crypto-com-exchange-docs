...
URL: https://exchange-developer.crypto.com/exchange/v1/docs/api/rest/public-get-expired-settlement-price
...
- `instrument_type` (string **required**) - FUTURE
- `page` (string (int32)) - Page number. Default 1.
...
- `data` (array of object)
  ...
  - `x` (string (int64)) - Expiry timestamp (ms).
...
```curl
curl -X GET "https://api.crypto.com/exchange/v1/public/get-expired-settlement-price?instrument_type=FUTURE&page=1" \
...
openapi: 3.0.3
info:
  title: public/get-expired-settlement-price
  version: 1.0.0
servers:
...
components:
  schemas:
    PublicGetExpiredSettlementPriceResponse:
      type: object
      description: |
        Response for public/get-expired-settlement-price (HTTP 200/4xx/5xx).
      ...
      properties:
        ...
        result:
          type: object
          description: Present when code === 0. data[] has i (instrument), x (expiry ms), v (value), t (timestamp).
          properties:
            data:
              type: array
              items:
                ...
                properties:
                  ...
                  x:
                    type: string
                    format: int64
                    description: Expiry timestamp (ms).
                  v:
                    type: string
                    format: decimal
                    description: Value

...
URL: https://exchange-developer.crypto.com/exchange/v1/docs/api/rest/private-advanced-cancel-oco

- **Method:** `POST`
- **Path:** `/private/advanced/cancel-oco`
...
paths:
  /private/advanced/cancel-oco:
    post:
      ...
      description: |
        Cancel an OCO order on the Exchange. This call is asynchronous, so the response is simply a confirmation of the request. The `user.advanced.order` subscription can be used to check when each of the orders is successfully cancelled.
      operationId: privateAdvancedCancelOco
      x-codeSamples: []
      requestBody:
        required: true
        content:
          application/json:
            schema:
              $ref: "#/components/schemas/PrivateAdvancedCancelOcoRequest"
            example:
              method: private/advanced/cancel-oco
              id: "1234"
              api_key: YOUR_API_KEY

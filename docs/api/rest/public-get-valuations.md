...
URL: https://exchange-developer.crypto.com/exchange/v1/docs/api/rest/public-get-valuations
...
- `instrument_name` (string **required**) - e.g. BTCUSD-INDEX
- `valuation_type` (string **required**) - **List of available types:**
...
- `count` (string (int32)) - Default is 25
- `start_ts` (string (int64)) - Default timestamp is 30 days ago for funding_hist, and 1 day ago for other valuation_type (Unix timestamp).
- `end_ts` (string (int64)) - Default timestamp is current time (Unix timestamp).
...
- `data` (array of object) - Array of value and timestamp (v, t).
  Array of objects:
...
```curl
curl -X GET "https://api.crypto.com/exchange/v1/public/get-valuations?instrument_name=BTCUSD-INDEX&valuation_type=index_price&count=1" \
...
components:
  schemas:
    PublicGetValuationsResponse:
      type: object
      description: |
        Response for public/get-valuations (HTTP 200/4xx/5xx).
      ...
      properties:
        ...
        result:
          type: object
          description: Present when code === 0. instrument_name, data array (v, t per item).
          properties:
            instrument_name:
              type: string
              description: e.g. BTCUSD-INDEX
            data:
              type: array
              description: Array of value and timestamp (v, t).
              items:
                $ref: "#/components/schemas/PriceTimestampItem"
        message:
          type: string

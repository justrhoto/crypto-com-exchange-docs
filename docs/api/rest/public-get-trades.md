...
URL: https://exchange-developer.crypto.com/exchange/v1/docs/api/rest/public-get-trades

- **Method:** `GET`
- **Path:** `/public/get-trades`
- **Tags:** Reference and Market Data

Fetches the public trades for a particular instrument. Time window can only be up to 7 days for maximum.
...
- `count` (string (int32)) - The maximum number of trades to be retrieved. Default: 25. Max: 150
- `start_ts` (string (int64)) - Start time in Unix time format (inclusive). Default: end_time - 1 day. Nanosecond recommended for accurate pagination
- `end_ts` (string (int64)) - End time in Unix time format (exclusive). Default: current system timestamp. Nanosecond recommended for accurate pagination
...
```curl
curl -X GET "https://api.crypto.com/exchange/v1/public/get-trades?instrument_name=BTCUSD-PERP&count=5" \
...
components:
  schemas:
    PublicGetTradesResponse:
      type: object
      description: |
        Response for public/get-trades (HTTP 200/4xx/5xx).
      ...
      properties:
        ...
        result:
          type: object
          description: Present when code === 0. Trades (data array, instrument_name). Items are PublicTradeItem (core + tn).
          properties:
            data:
              type: array
              items:
                $ref: "#/components/schemas/PublicTradeItem"
        message:
          type: string
    ...
    PublicTradeItem:
      description: One item in REST public/get-trades result.data[]. Extends MarketTradeItemBase with tn.
      allOf:
        - $ref: "#/components/schemas/MarketTradeItemBase"
        - type: object
          properties:
            tn:
              type: string
              format: int64
              description: Trade timestamp in nanoseconds (REST only)
              e.g. \"1771761038000000000\".: null
    MarketTradeItemBase:
      type: object
      description: |

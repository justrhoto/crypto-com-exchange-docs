...
URL: https://exchange-developer.crypto.com/exchange/v1/docs/api/rest/private-get-transactions
...
- `isolation_id` (string) - Optional filter for transactions of a particular isolated position. Omit for 'all'.
- `journal_type` (enum: TRADING | TRADE_FEE | ONCHAIN_WITHDRAWAL | ONCHAIN_DEPOSIT | FUNDING | REALIZED_PNL | INSURANCE_FUND | SOCIALIZED_LOSS | LIQUIDATION_FEE | SESSION_RESET | ADJUSTMENT | SESSION_SETTLE | UNCOVERED_LOSS | ADMIN_ADJUSTMENT | DELIST | SETTLEMENT_FEE | AUTO_CONVERSION | MANUAL_CONVERSION | SUBACCOUNT_TX | FIAT_WITHDRAWAL_CANCEL | MARGIN_TRADE_INTEREST)
- `limit` (integer (int32)) - Maximum number of transactions to retrieve before end_time. Default: 100. Max: 100.
...
- `data` (array of object) - List of transactions.
  Array of objects:
  - `account_id` (string) - Account ID.
  - `client_oid` (string) - Client Order ID (can be empty).
  - `event_date` (string) - Event date.
  - `event_timestamp_ms` (string (int64)) - Event timestamp in milliseconds.
  - `event_timestamp_ns` (string (int64)) - Event timestamp in nanoseconds.
  - `instrument_name` (string) - E.g. BTCUSD-PERP.
  - `isolation_id` (string) - Isolation ID of the order if under isolated position.
  - `isolation_type` (enum: ISOLATED_MARGIN) - Isolation type
  - `journal_id` (string (int64)) - Journal ID.
  - `journal_type` (enum: TRADING | TRADE_FEE | ONCHAIN_WITHDRAWAL | ONCHAIN_DEPOSIT | FUNDING | REALIZED_PNL | INSURANCE_FUND | SOCIALIZED_LOSS | LIQUIDATION_FEE | SESSION_RESET | ADJUSTMENT | SESSION_SETTLE | UNCOVERED_LOSS | ADMIN_ADJUSTMENT | DELIST | SETTLEMENT_FEE | AUTO_CONVERSION | MANUAL_CONVERSION | SUBACCOUNT_TX | FIAT_WITHDRAWAL_CANCEL | MARGIN_TRADE_INTEREST) - Journal / transaction type
  - `order_id` (string (int64)) - Order ID.
  - `realized_pnl` (string (decimal)) - Realized PNL.
  - `side` (enum: BUY | SELL) - BUY or SELL
  - `taker_side` (enum: MAKER | TAKER)
  - `trade_id` (string (int64)) - Trade ID.
  - `trade_match_id` (string (int64)) - Trade match ID; applicable to trades only; non-trade transactions may have zero or null.
  - `transaction_cost` (string (decimal)) - Transaction cost.
  - `transaction_qty` (string (decimal)) - Transaction quantity.
  ...
  schemas:
    PrivateGetTransactionsRequest:
      ...
      required:
        ...
        - nonce
        - params
      description: Request body for private/get-transactions.
      properties:
        id:
          type: string
          format: int64
        ...
        method:
          type: string
          description: Must be "private/get-transactions"
          example: private/get-transactions
        api_key:
          type: string
          ...
          properties:
            ...
            journal_type:
              allOf:
                ...
                - description: Filter by journal type. E.g. TRADING, TRADE_FEE, ...
            start_time:
              type: string
              format: int64
            ...
            limit:
              type: integer
              format: int32
              description: "Maximum number of transactions to retrieve before end_time. Default: 100. Max: 100."
            isolation_id:
              type: string
              description: Optional filter for transactions of a particular isolated position. Omit for 'all'.
    JournalType:
      type: string
      enum:
        - TRADING
    ...
    PrivateGetTransactionsResponse:
      type: object
      description: |
        Response for private/get-transactions (HTTP 200/4xx/5xx).
      ...
      properties:
        ...
        result:
          type: object
          description: Present when code === 0. result.data[] per docs (journal_type, journal_id, transaction_qty, realized_pnl, etc.).
          properties:
            data:
              type: array
              ...
              items:
                ...
                properties:
                  ...
                  trade_match_id:
                    type: string
                    format: int64
                    description: Trade match ID; applicable to trades only; non-trade transactions may have zero or null.
                  event_timestamp_ms:
                    type: string
                    format: int64
                  ...
                  client_oid:
                    type: string
                    description: Client Order ID (can be empty).
                  taker_side:
                    allOf:

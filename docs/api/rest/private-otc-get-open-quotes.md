...
URL: https://exchange-developer.crypto.com/exchange/v1/docs/api/rest/private-otc-get-open-quotes

- **Method:** `POST`
- **Path:** `/private/otc/get-open-quotes`
...
- `lp_quote_id` (string) - LP supplied Quote ID; if provided
- `quote_id` (string) - System supplied Quote ID; if provided
...
- `count` (string (int32)) - Number of quotes returned
- `quote_list` (array of object) - List of open quotes (status ACTIVE).
  Array of objects:
  - `expire_time_ns` (string) - Alias for expiry_time_ns (Unix epoch time when quote expires in nanosecond).
  - `expiry_time_ns` (string) - Unix epoch time when quote expires in nanosecond
  - `leg_list` (array of object)
    Array of objects:
    - `ask` (string (decimal)) - The ask price for the instrument
    - `bid` (string (decimal)) - The bid price for the instrument
    ...
    - `type` (enum: PRICE) - PRICE. Only present when the quote has pricing (i.e. bid / ask).
  - `lp_quote_id` (string) - LP supplied ID for the quote
  ...
  - `response_time_ns` (string) - Unix epoch time when quote was received in nanosecond
...
openapi: 3.0.3
info:
  title: private/otc/get-open-quotes
  version: 1.0.0
servers:
  ...
  schemas:
    PrivateOtcGetOpenQuotesRequest:
      ...
      required:
        ...
        - nonce
        - params
      description: "Request body for private/otc/get-open-quotes. Get open quotes (active only: status = ACTIVE)."
      properties:
        id:
          type: string
          format: int64
        ...
        method:
          type: string
          description: Must be "private/otc/get-open-quotes"
          example: private/otc/get-open-quotes
        api_key:
          type: string
        ...
        params:
          type: object
          description: If quote_id or lp_quote_id provided, query ignores time range.
          properties:
            quote_id:
              type: string
              description: System supplied Quote ID; if provided
              query will ignore time range: null
            lp_quote_id:
              type: string
              description: LP supplied Quote ID; if provided
              query will ignore time range: null
            start_time:
              type: string
              format: int64
    ...
    PrivateOtcGetOpenQuotesResponse:
      type: object
      description: |
        Response for private/otc/get-open-quotes (HTTP 200/4xx/5xx).
        code === 0 → success, result present with count and quote_list.
      ...
      properties:
        ...
        result:
          ...
          properties:
            ...
            quote_list:
              type: array
              description: List of open quotes (status ACTIVE).
              items:
                type: object
                properties:
                  quote_id:
                  ...
                  lp_quote_id:
                    type: string
                    description: LP supplied ID for the quote
                  quote_req_id:
                    type: string
                  ...
                  response_time_ns:
                    type: string
                    description: Unix epoch time when quote was received in nanosecond
                  expiry_time_ns:
                    type: string
                    description: Unix epoch time when quote expires in nanosecond
                  expire_time_ns:
                    type: string
                    description: Alias for expiry_time_ns (Unix epoch time when quote expires in nanosecond).
                  leg_list:
                    type: array
                    items:
                      $ref: "#/components/schemas/PrivateOtcGetOpenQuotesQuoteListLegItem"
        message:
          type: string
        ...
        original:
          type: string
          description: Present when code !== 0.
    PrivateOtcGetOpenQuotesQuoteListLegItem:
      type: object
      description: One leg in an open quote. Either quantity or notional; bid/ask when quote has pricing.
      properties:
        instrument_name:
          type: string
        ...
        bid:
          type: string
          format: decimal
          description: The bid price for the instrument
        ask:
          type: string
          format: decimal
          description: The ask price for the instrument
        type:
          type: string
          enum:
            - PRICE
          description: PRICE. Only present when the quote has pricing (i.e. bid / ask).
```

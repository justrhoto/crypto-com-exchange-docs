...
URL: https://exchange-developer.crypto.com/exchange/v1/docs/api/rest/public-get-insurance
...
- `instrument_name` (string **required**) - e.g. USD
- `count` (string (int32)) - Default is 25
- `start_ts` (string (int64)) - Default timestamp is 1 day ago (Unix timestamp).
...
```curl
curl -X GET "https://api.crypto.com/exchange/v1/public/get-insurance?instrument_name=USD&count=1" \
...
components:
  schemas:
    PublicGetInsuranceResponse:
      type: object
      description: |
        Response for public/get-insurance (HTTP 200/4xx/5xx).
      ...
      properties:
        ...
        result:
          type: object
          description: Present when code === 0. instrument_name (e.g. USD), data[] has v (value), t (timestamp).
          properties:
            instrument_name:
              type: string
              description: e.g. USD
